...
import { MessageSquare, Users, Hash, Search, UserPlus } from 'lucide-react';
...
const Chat = () => {
  ...
  // Deduplicate direct conversations by counterpart (WhatsApp-style)
  const displayConversations = useMemo(() => {
    const map = new Map<string, any>();

    conversations.forEach((c: any) => {
      if (c.type === 'group') {
        map.set(`group:${c.id}`, c);
        return;
      }
      const other = c.participants?.find((p: any) => p.user_id !== user?.id);
      const key = other?.user_id || c.id;
      const existing = map.get(key);
      if (!existing) {
        map.set(key, c);
      } else {
        const newer = new Date(c.created_at) > new Date(existing.created_at) ? c : existing;
        map.set(key, newer);
      }
    });
